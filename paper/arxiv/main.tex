...
We have run \texttt{keda-gpu-scaler} on a four-node GPU cluster (8$\times$A100
80\,GB per node, on Amazon EKS) serving LLM inference with vLLM (Llama~3 70B,
4-bit quantized). We observed the following. First, device memory is the right
...
staging endpoints sat idle more than 18 hours per day, and releasing those GPUs
reduced non-production GPU spend by roughly 60\%. Third, the short poll interval
